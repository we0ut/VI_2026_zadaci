...
if __name__ == '__main__':
    ...
    split0 = int(0.8 * len(class0))
    split1 = int(0.8 * len(class1))

    split_index = int(0.8 * len(dataset))

    X, y =[row[:-1] for row in dataset], [row[-1]for row in dataset]
    # train_set, val_set = class0[:split0] + class1[:split1], class0[split0:] + class1[split1:]
    # train_set, val_set = X[split:],


    # train_X, val_X = [row[:-1] for row in train_set], [row[:-1] for row in val_set]
    # train_y, val_y = [row[-1] for row in train_set], [row[-1] for row in val_set]
    train_X, train_y = X[:split_index], y[:split_index]
    val_X, val_y = X[split_index:], y[split_index:]
    ...
    model_1.fit(scaled_train_X, train_y)

    preds_train = model_1.predict(scaled_train_X)
    preds_val = model_1.predict(scaled_val_X)

    # acc_train = accuracy_score(train_y, preds_train)
    # acc_val = accuracy_score(val_y, preds_val)

    TP, TN, FP, FN = 0, 0, 0, 0
    for pred, actual in zip(preds_train, train_y):
        if pred == 1 and pred == actual:
            TP += 1
        if pred == 1 and pred != actual:
            FP += 1
        if pred == 0 and pred == actual:
            TN += 1
        if pred == 0 and pred != actual:
            FN += 1
    ...
    TP, TN, FP, FN = 0, 0, 0, 0
    for pred, actual in zip(preds_val, val_y):
        if pred == 1 and pred == actual:
            TP += 1
        if pred == 1 and pred != actual:
            FP += 1
    ...
    if acc_train > acc_val*1.15:
        ...
        train_X = [row[:col_index] + row[col_index + 1:] for row in train_X]
        val_X = [row[:col_index] + row[col_index + 1:] for row in val_X]
        ...
        model_2 = MLPClassifier(**params)
        model_2.fit(scaled_train_X, train_y)

        prediction = model_2.predict(new_sample)[0]
    else:
        new_sample = scaler.transform([new_sample])
        prediction = model_1.predict(new_sample)[0]
