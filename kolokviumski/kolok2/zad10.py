...
if __name__ == '__main__':
    ...
    x = int(input())
    # class1 = [row for row in dataset if row[-1] >= 5]
    # class0 = [row for row in dataset if row[-1] < 5]

    # split_idx_0 = int(x/100 * len(class0))
    # split_idx_1 = int(x/100 * len(class1))
    split_idx = int(x/100 * len(dataset))
    # train_set, test_set = class0[split_idx_0:] + class1[split_idx_1:], class0[:split_idx_0] + class1[:split_idx_1]
    ...
    train_y, test_y = [1 if row[-1] >= 5 else 0  for row in train_set], [1 if row[-1] >= 5 else 0  for row in test_set]
    ...
    # print(min_index)
    train_X_new, test_X_new = [row[:min_index] + row[min_index+1:] for row in train_X], [row[:min_index] + row[min_index+1:] for row in test_X]
    ...
    modelMLP = MLPClassifier(**paramsmlp)

    standard_scaler = StandardScaler()
    minmax_scaler = MinMaxScaler()

    scaled_train_X = standard_scaler.fit_transform(train_X_new)
    scaled_test_X = standard_scaler.transform(test_X_new)
    # scaled_train_y = standard_scaler.fit_transform(train_y)
    # scaled_test_y = standard_scaler.transform(test_y)

    minmax_train_X = minmax_scaler.fit_transform(train_X_new)
    minmax_test_X = minmax_scaler.transform(test_X_new)

    modelMLP.fit(scaled_train_X, train_y)
    print(f'Tocnost so StandardScaler: {modelMLP.score(scaled_test_X, test_y)}')

    modelMLP = MLPClassifier(**paramsmlp)
    modelMLP.fit(minmax_train_X, train_y)
    print(f'Tocnost so MinMaxScaler: {modelMLP.score(minmax_test_X, test_y)}')
