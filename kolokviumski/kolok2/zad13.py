...
if __name__ == '__main__':
    ...
    # za NB
    split0 = int(len(class0) * x1)
    split1 = int(len(class1) * x1)
    split2 = int(len(class2) * x1)

    # X_nb, y_nb = [row[:-1] for row in dataset], [row[-1] for row in dataset]
    train_set = class0[:split0] + class1[:split1] + class2[:split2]
    # test_set = class0[split0:] + class1[split1:] + class2[split2:]

    train_X_nb, train_y_nb = [row[:-1] for row in train_set], [row[-1] for row in train_set]
    # test_X_nb, test_y_nb = [row[:-1] for row in test_set], [row[-1] for row in test_set]

    modelNB = GaussianNB()
    modelNB.fit(train_X_nb, train_y_nb)

    # za DT
    split0_x2 = int(len(class0) * x2)
    split1_x2 = int(len(class1) * x2)
    split2_x2 = int(len(class2) * x2)

    # X_nb, y_nb = [row[:-1] for row in dataset], [row[-1] for row in dataset]
    train_set = class0[split0:split0_x2] + class1[split1:split1_x2] + class2[split2:split2_x2]
    # test_set = class0[:split0] + + class1[split1:] + class2[split2:]

    train_X_dt, train_y_dt = [row[:-1] for row in train_set], [row[-1] for row in train_set]
    # test_X_nb, test_y_nb = [row[:-1] for row in test_set], [row[-1] for row in test_set]

    modelDT = DecisionTreeClassifier()
    modelDT.fit(train_X_dt, train_y_dt)

    # za RF
    train_set = class0[:split0_x2] + class1[:split1_x2] + class2[:split2_x2]

    train_X_rf, train_y_rf = [row[:-1] for row in train_set], [row[-1] for row in train_set]

    modelRF = RandomForestClassifier(n_estimators=3)
    modelRF.fit(train_X_rf, train_y_rf)

    test_set = class0[split0_x2:] + class1[split1_x2:] + class2[split2_x2:]
    test_X, test_y = [row[:-1]for row in test_set], [row[-1] for row in test_set]
    # print(test_X)

    tocen_zapis=0
    preds_nb = modelNB.predict(test_X)
    preds_dt = modelDT.predict(test_X)
    preds_rf = modelRF.predict(test_X)
    for pred_nb,pred_dt,pred_rf, actual in zip(preds_nb, preds_dt, preds_rf, test_y):
        count = 0
        if pred_nb == actual:
            count += 1
        if pred_dt == actual:
