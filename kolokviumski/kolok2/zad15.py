...
if __name__ == '__main__':
    ...
    split0 = int(x_num/100 * len(class0))
    split1 = int(x_num/100 * len(class1))

    train_set, test_set = class0[:split0] + class1[:split1], class0[split0:] + class1[split1:]
    ...
    model_rf = RandomForestClassifier(n_estimators=4, random_state=0)
    ...
    acc_nb = model_nb.score(test_X, test_y)
    acc_dt = model_dt.score(test_X, test_y)
    acc_rf = model_rf.score(test_X, test_y)
    acc_nn = model_nn.score(test_X, test_y)

    accs_text = ['Naive Bayes', 'Decision Tree', 'Random Forest', 'MLP']
    accs = [acc_nb, acc_dt, acc_rf, acc_nn]
    v_arr = [1,1,1,1]
    accs_max_index = accs.index(max(accs))

    print(f'Najgolema tocnost ima klasifikatorot {accs_text[accs_max_index]}')
    v_arr[accs_max_index] += 1


    pred_nb = model_nb.predict(test_X)
    pred_dt = model_dt.predict(test_X)
    pred_rf = model_rf.predict(test_X)
    pred_nn = model_nn.predict(test_X)

    votes = (pred_nb*v_arr[0]) + (pred_dt * v_arr[1]) + (pred_rf * v_arr[2]) + (pred_nn * v_arr[3])
    ...
    recall = recall_score(test_y, final)
